...
@section('content')
    <div class="container">
        ...
        @if (session()->has('panier'))
                    ...
                    <tbody>
                        ...
                        @foreach (session('panier') as $key => $article)
                            ...
                            <tr>
                                ...
                                <td>
                                    <strong><a href="{{ route('article.show', $key) }}"
                                        ...
                                        <div class="row">
                                            <div class="col-12 d-flex">
                                                <input type="number" min="1" max="10" name="quantite"
                                                    placeholder="Quantité ?" value="{{ $article['quantite'] }}"
                                ...
                                <td>
                                    ...
                                    <a href="{{ route('panier.remove', $key) }}" class="button-26"
                                        title="Retirer le produit du panier" style="text-decoration:none; ">Supprimer</a>
                                </td>
                            </tr>
                        @endforeach
                        <tr colspan="2">
            ...
            <a class="button-24" href="{{ route('panier.empty') }}" title="Retirer tous les produits du panier"
                style="text-decoration:none; ">Vider le panier</a>
        ...
        @endif
        <form action="{{ route('choixadresse') }}" method="post" class="bg-white shadow w-50 p-5 mx-auto mt-5">
            @csrf
            <div class="mb-3">
        ...
        <div class="bg-white shadow w-50 p-5 mx-auto mt-5">
            ...
            <form action="{{ route('choixlivraison') }}" method="post">
                @csrf
                <div>
                    <input type="radio" id="classic" name="livraison" value="classic" class="me-2"
                        @if (isset($livraison) && $livraison === 'classic') checked @endif>
                    <label for="classic">Classique (à domicile, 48h) : 5.00 €</label>
                </div>

                <div class="mt-2">
                    <input type="radio" id="express" name="livraison" value="express" class="me-2"
                        @if (isset($livraison) && $livraison === 'express') checked @endif>
                    <label for="express">Express (à domicile, 24h) : 9,90 €</label>
                </div>

                <div class="mt-2">
                    <input type="radio" id="relais" name="livraison" value="relais" class="me-2"
                        @if (isset($livraison) && $livraison === 'relais') checked @endif>
        ...
        <div class="bg-white shadow w-50 p-5 mx-auto mt-5 text-center">
            @if (isset($nouveautotal) && (session()->get('adresse')))
            ...
            @else
                <h1>Choissez une adresse et un mode de livraison pour connaître le total.</h1>
            @endif
        </div>
